...
function ProductsAdminPage({ onLogout, showViewSwitch = false, adminView = "admin", onAdminViewChange }) {
  ...
  const uploadImageIfNeeded = async () => {
    ...
    if (!response.ok) {
      throw new Error(data.detail || "Resim yuklenemedi");
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!response.ok) {
        setMessage(data.detail || "Islem basarisiz.");
        return;
      }
  ...
  const handleDelete = async (productId) => {
    ...
    try {
      ...
      if (!response.ok) {
        setMessage(data.detail || "Silme islemi basarisiz.");
        return;
      }
  ...
  return (
    <div style={styles.container}>
        ...
        {products.length === 0 ? (
          ...
          products.map((product) => (
            <div key={product.id} style={styles.card}>
              ...
              <div style={styles.buttonRow}>
                <button onClick={() => handleEdit(product)} style={styles.editButton}>Duzenle</button>
                <button onClick={() => handleDelete(product.id)} style={styles.deleteButton}>Sil</button>
              </div>
            </div>
          ))
        )}
      ...
      <div style={styles.bottomActions}>
        <button onClick={onLogout} style={styles.logoutButton}>Cikis Yap</button>
      </div>
    </div>
  );
}
